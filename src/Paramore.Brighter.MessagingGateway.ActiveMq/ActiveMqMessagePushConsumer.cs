...
public class ActiveMqMessagePushConsumer(ChannelReader<IMessage> channel, RoutingKey defaultTopic)
    : IAmAMessageConsumerSync, IAmAMessageConsumerAsync
{
    ...
    public async Task PurgeAsync(CancellationToken cancellationToken = default)
    {
        while (true)
        {
            try
            {
                ...
                var message = await channel.ReadAsync(cts.Token);
    ...
    private Message ToBrighterMessage(IMessage message)
    {
        ...
        RoutingKey GetTopic()
        {
            ...
            return string.IsNullOrEmpty(topic) ? defaultTopic : new RoutingKey(topic);
        }
