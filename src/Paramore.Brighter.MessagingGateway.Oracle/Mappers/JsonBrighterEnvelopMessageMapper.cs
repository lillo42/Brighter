...
public class JsonBrighterEnvelopMessageMapper<TRequest> : IAmAMessageMapper<TRequest>, IAmAMessageMapperAsync<TRequest> where TRequest : class, IRequest
{
    ...
    public Message MapToMessage(TRequest request, Publication publication)
    {
        ...
        var body = new MessageBody(JsonSerializer.Serialize(new BrighterEnvelop
        {
            Id = request.Id,
            Source = publication.Source,
            Type = publication.Type,
            ...
            DataSchema = publication.DataSchema,
            AdditionalProperties = defaultCloudEventsAdditionalProperties
    ...
    {
        var request = JsonSerializer.Deserialize<BrighterEnvelop>(message.Body.Value, JsonSerialisationOptions.Options);
